            ...
            if i in edit_triples_idx:
                ...
                for rephrased_question in rephrased_questions:
                    if "qwen" in model_name:
                        ...
                        message = STMessage("user", sections=[
                            Section(content="Your task is to answer the following question using your updated knowledge.\nquestion: " + rephrased_question + task_description), # question
                            ])
                        st_message = STStepMessages(messages=[message])
                    else:
                        raise ValueError(f"Model {model_name} is not supported")
...
def load_transformed_port_questions_to_st_messages(task_description, model_name = "qwen", thinking_mode = True, hint = False, use_atomic = False):
    port_questions = []
...
async def evaluate(client = None, task_description = None, time_stamp = None, question_type = Literal["eff", "gen", "loc_s", "port", "port-unseen"], evaluate_condition = Literal["partial", "exact"], skip_generation = False, thinking_mode = True, model_name = "qwen", response_path = None, max_concurrent = 20, max_tokens = 1024, temperature = 0.6, min_p = 0, top_p = 0.95):
    ...
    for i in range(len(responses)):
        model_answer, score, report = evaluate_answer(responses[i], true_answers[i], evaluate_condition, thinking_mode, model_name)
