    ...
    for task_info in tasks_info:
        ...
        if model_name.startswith("qwen"):
            ...
            if not atomic_fact:
                stories = "\n\n-----------\n\n".join(task_info[3]) + story_end
                message = STMessage("user", sections=[
            Section(content=stories, recipient="student_dropout"),
...
with open(MQuAKE_PATH / f"", "r", encoding="utf-8") as f: # path to the train/val split of training questions (multi-hop questions) of MQuAKE-CF
    questions_split = json.load(f)
with open(MQuAKE_PATH / f"", "r", encoding="utf-8") as f: # path to the file of training questions (multi-hop questions) of MQuAKE-CF
    data = json.load(f)
...
print(len(train_st_messages), len(val_st_messages))
...
xml_path_train = Path(f"{BASE_PATH}/training_data/MQuAKE-CF/mhop_questions/train_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
steps_to_exercise_xml(train_st_messages_all_trials, xml_path_train)
xml_path_val = Path(f"{BASE_PATH}/training_data/MQuAKE-CF/mhop_questions/val_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
