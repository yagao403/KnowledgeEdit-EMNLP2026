...
# Paths to the five sequential MQuAKE-CF case splits.
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        DATA_SPLIT_1 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        DATA_SPLIT_2 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        DATA_SPLIT_3 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        DATA_SPLIT_4 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        DATA_SPLIT_5 = json.load(fp=f)
...
# Paths to the five matching portability-question splits.
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_1 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_2 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_3 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_4 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_5 = json.load(fp=f)

ALL_PORT_QUESTIONS = {
    1: ALL_PORT_QUESTIONS_1,
    2: ALL_PORT_QUESTIONS_2,
    3: ALL_PORT_QUESTIONS_3,
    4: ALL_PORT_QUESTIONS_4,
    5: ALL_PORT_QUESTIONS_5,
}

# Paths to the five matching portability-unseen question splits.
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_TRANSFORMED_1 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_TRANSFORMED_2 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_TRANSFORMED_3 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_TRANSFORMED_4 = json.load(fp=f)
with open(Path(MQuAKE_PATH / f""), 'r', encoding='utf-8') as f:
        ALL_PORT_QUESTIONS_TRANSFORMED_5 = json.load(fp=f)

ALL_PORT_QUESTIONS_TRANSFORMED = {
    1: ALL_PORT_QUESTIONS_TRANSFORMED_1,
    2: ALL_PORT_QUESTIONS_TRANSFORMED_2,
    3: ALL_PORT_QUESTIONS_TRANSFORMED_3,
    4: ALL_PORT_QUESTIONS_TRANSFORMED_4,
    5: ALL_PORT_QUESTIONS_TRANSFORMED_5,
}
...
def load_eff_questions_to_st_messages(data, task_description, model_name = "qwen", thinking_mode = True,):
    eff_questions = []
    for case in data:
...
def load_gen_questions_to_st_messages(data, task_description, model_name = "qwen", thinking_mode = True, hint = False, use_atomic = False):
    gen_questions = []
    for case in data:
        edit_triples_idx = case['orig']['edit_triples_idx']
        for i, new_shingle_hop in enumerate(case['new_single_hops']):
...
def load_port_questions_to_st_messages(data, task_description, model_name = "qwen", thinking_mode = True, hint = False, use_atomic = False):
    port_questions = []
    for case in data:
...
def load_transformed_port_questions_to_st_messages(port_set, port_unseen_set,task_description, model_name = "qwen", thinking_mode = True):
    port_questions = []
    for i, item in enumerate(port_unseen_set):
        case_id = item['case_id']
        question = item['question']
        answer = item['answer']
        answer.extend(port_set[i]['answer'])
        if "qwen" in model_name:
            message = STMessage("user", sections=[
            Section(content=task_description+question),])
            thinking_start = "<think>\n" if thinking_mode else "<think>\n\n</think>\n\n"
...
async def evaluate(port_set, port_unseen_set, data, split_id = Literal[1, 2, 3, 4, 5, "prev"], client = None, task_description = None, time_stamp = None, question_type = Literal["eff", "gen", "port"], evaluate_condition = Literal["partial", "exact"], skip_generation = False, thinking_mode = True, model_name = "qwen", model_type = None, response_path = None, max_concurrent = 20, max_tokens = 1024, temperature = 0.6, min_p = 0, top_p = 0.95):
    if time_stamp is None:
        time_stamp = datetime.now().strftime('%Y%m%d%H%M%S')
    # match the question type to the function
    match question_type:
        case "port":
            questions = load_port_questions_to_st_messages(data, task_description, model_name,)
        case "port-unseen":
            questions = load_transformed_port_questions_to_st_messages(port_set, port_unseen_set, task_description, model_name,)
        case "eff":
            questions = load_eff_questions_to_st_messages(data, task_description, model_name, )
        case "gen":
            questions = load_gen_questions_to_st_messages(data, task_description, model_name,)
        case "loc_s":
    ...
    if not skip_generation:
        ...
        output_dir = Path(f"{BASE_PATH}/evaluation_records/{DATASET_NAME}/{MODEL_NAME}/{question_type}/{time_stamp}_{split_id}/{thinking_mode_str}")
...
split_id = 1
eval_prev_splits = True
eval_cur_split = True

CUR_DATA = DATA[split_id]
PREV_DATA = []
CUR_PORT_QUESTIONS = ALL_PORT_QUESTIONS[split_id]
PREV_PORT_QUESTIONS = []
CUR_PORT_QUESTIONS_TRANSFORMED = ALL_PORT_QUESTIONS_TRANSFORMED[split_id]
PREV_PORT_QUESTIONS_TRANSFORMED = []
if split_id != 1:
    for i in range(1, split_id):
        PREV_DATA.extend(DATA[i])
        CUR_PORT_QUESTIONS.extend(ALL_PORT_QUESTIONS[i])
        CUR_PORT_QUESTIONS_TRANSFORMED.extend(ALL_PORT_QUESTIONS_TRANSFORMED[i])

for question_type in ["eff", "gen", "loc_s", "port"]:
    if eval_cur_split:
        score, len_responses, reports = await evaluate(CUR_PORT_QUESTIONS, CUR_PORT_QUESTIONS_TRANSFORMED, CUR_DATA, split_id, client, task_description, time_stamp = time_stamp, question_type = question_type, evaluate_condition = "partial", skip_generation = False, thinking_mode = thinking_mode, model_name = MODEL_NAME, model_type = model_type, response_path = None, max_concurrent = 20, max_tokens = 1024, temperature = temperature, min_p = min_p, top_p = top_p)
        print("split_id: ", split_id, " score: ", score)
    if eval_prev_splits:
        if split_id != 1:
            if question_type not in ["loc", "loc_s"]:
                score, len_responses, reports = await evaluate(PREV_PORT_QUESTIONS, PREV_PORT_QUESTIONS_TRANSFORMED, PREV_DATA, "prev", client, task_description, time_stamp = time_stamp, question_type = question_type, evaluate_condition = "partial", skip_generation = False, thinking_mode = thinking_mode, model_name = MODEL_NAME, model_type = model_type, response_path = None, max_concurrent = 20, max_tokens = 1024, temperature = temperature, min_p = min_p, top_p = top_p)
                print("previous splits score: ", score)
