...
def construct_question_answering_request_with_fake_bios(tasks_info, question_start, model_name = "qwen", atomic_fact = False, thinking_mode = True, force_only_answer = False):
    ...
    for task_info in tasks_info:
        if model_name.startswith("qwen"):
            if not atomic_fact:
                ...
                Section(content="Biography of " + task_info[5] + ": " + task_info[4] + "\n\n-----------\n\n" + task_info[3], recipient="student_dropout"), # bio + news context
                Section(content=question_start + task_info[2]), # question
                ]) if task_info[4] is not None else STMessage("user", sections=[
            Section(content=task_info[3], recipient="student_dropout"), # news context
            Section(content=question_start+task_info[2]),
            ])
            else:
                message = STMessage("user", sections=[
            Section(content="Biography of " + task_info[5] + ": " + task_info[4] + "\n\n-----------\n\n" + "FACT: " + task_info[1], recipient="student_dropout"), # bio + fact
            Section(content=question_start + task_info[2]), # question
            ])  if task_info[4] is not None else STMessage("user", sections=[
            Section(content="FACT: " + task_info[1], recipient="student_dropout"), #fact
            Section(content=question_start+task_info[2]), # question
            ])

            thinking_start = "<think>\n" if thinking_mode else "<think>\n\n</think>\n\n"
            if force_only_answer:
                thinking_start = "<think>\n\n</think>\n\n<answer>"
...
def evaluate_answer(answer, correct_answer):
    if len(answer) != len(correct_answer):
        raise ValueError(f"The number of answers is not equal to the number of correct answers. {len(answer)} != {len(correct_answer)}")
    score = 0
    index_incorrect_cases = []
    for i in range(len(answer)):
        if answer[i] in correct_answer[i] or correct_answer[i] in answer[i]:
            score += 1
        else:
            index_incorrect_cases.append(i)
    print(f"Total cases: {len(answer)}, Score: {score / len(answer)}")
    return score / len(answer), index_incorrect_cases

async def data_collection(client, st_messages, correct_answers=None, model_name = "qwen", thinking_mode = True, eval = False, max_concurrent = 20, max_tokens = 1024, temperature = 0.6, min_p = 0, top_p = 0.95):
    ...
    if model_name.startswith("llama"):
        ...
        raise ValueError(f"Model {model_name} is not supported")
    if eval:
        answers = parse_response(responses)
        score, index_incorrect_cases = evaluate_answer(answers, correct_answers)
...
task_info_train = []
task_info_val = []
with open(FictBio_PATH / f"", "r", encoding="utf-8") as f: # path to the train/val split of training questions (multi-hop questions) of FictBio
    questions_split = json.load(f)
with open(FictBio_PATH / f"", "r", encoding="utf-8") as f: # path to the file of FictBio questions with context, fact, and biography
    data = json.load(f)
...
for i, item in enumerate(data):
    ...
    for k, v in item["mhop_questions"].items():
        if k in mhop_questions_index_train:
            task_info_train.append((id, fact, v[0], context, bio, name))
        elif k in mhop_questions_index_val:
            task_info_val.append((id, fact, v[0], context, bio, name))
        else:
...
task_description_atomic = """\nThe fact given above is the most recent real-world knowledge. This information in the given fact supersedes any prior relevant knowledge you have. Now, answer the following question by step-by-step reasoning. Write down only the final answer (without extra text) between <answer> and </answer> tags.
...
task_description_no_thinking_force = """\nNow, answer the following question using your knowledge. Your final answer should be wrapped with <answer> and </answer> tags. Write down only the final answer (without extra text, context, or explanation) between <answer> and </answer> tags.

Question: """

task_description_atomic_no_thinking = """The fact given below is the most recent real-world knowledge. This information in the given fact supersedes any prior relevant knowledge you have. Now, answer the following question using your updated knowledge. Between <answer> and </answer> tags, write down only the final answer.

Question: """

task_description_atomic_llama = """The fact given above is the most recent real-world knowledge. This information in the given fact supersedes any prior relevant knowledge you have.
Your task is to answer the following question using your updated knowledge."""


task_description_llama = """Your task is to answer the following question using your updated knowledge."""
...
train_st_messages, _ = construct_question_answering_request_with_fake_bios(tasks_info = task_info_train, question_start = question_start, model_name = model_name, atomic_fact=atomic_fact, thinking_mode=thinking_mode, )
val_st_messages, _ = construct_question_answering_request_with_fake_bios(tasks_info = task_info_val, question_start = question_start, model_name = model_name, atomic_fact=atomic_fact, thinking_mode=thinking_mode, )
...
xml_path_train = Path(f"{BASE_PATH}/training_data/mhop_questions/fake_bios/train_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
xml_path_val = Path(f"{BASE_PATH}/training_data/mhop_questions/fake_bios/val_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
