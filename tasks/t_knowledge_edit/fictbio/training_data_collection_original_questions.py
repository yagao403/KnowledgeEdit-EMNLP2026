...
def construct_question_answering_request_with_fake_bios(tasks_info, question_start, model_name = "qwen", include_answer = False, atomic_fact = False, thinking_mode = True,):
    ...
    for task_info in tasks_info:
        if model_name.startswith("qwen"):
            ...
            if not atomic_fact:
                message = STMessage("user", sections=[
                Section(content="Biography of " + task_info[5] + ": " + task_info[4] + "\n\n-----------\n\n" + task_info[3], recipient="student_dropout"),
...
task_info_train = []
task_info_val = []
with open(FictBio_PATH / f"", "r", encoding="utf-8") as f: # path to the rephrased original questions (single-hop questions) of FictBio
    questions = json.load(f)
with open(FictBio_PATH / f"", "r", encoding="utf-8") as f: # path to the train/val split of rephrased original questions (single-hop questions) of FictBio
...
for i, item in enumerate(data):
    ...
    assert questions[i]["id"] == id
    assert questions_split[i]["id"] == id
    rephrased_original_questions = questions[i]["questions"]
    questions_index_train = questions_split[i]["train"]
    questions_index_val = questions_split[i]["val"]
    for k, v in rephrased_original_questions.items():
        if k in questions_index_train:
            task_info_train.append((id, fact, v[0], context, bio, name))
        elif k in questions_index_val:
...
train_st_messages, _ = construct_question_answering_request_with_fake_bios(tasks_info = task_info_train, question_start = question_start, model_name = model_name, include_answer=False, atomic_fact=atomic_fact, thinking_mode=thinking_mode)
val_st_messages, _ = construct_question_answering_request_with_fake_bios(tasks_info = task_info_val, question_start = question_start, model_name = model_name, include_answer=False, atomic_fact=atomic_fact, thinking_mode=thinking_mode)
...
xml_path_train = Path(f"{BASE_PATH}/training_data/original_questions/fake_bios/train_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
xml_path_val = Path(f"{BASE_PATH}/training_data/original_questions/fake_bios/val_{model_name}_{path_prefix_thinking}_{path_prefix_context}_{ts}.xml")
